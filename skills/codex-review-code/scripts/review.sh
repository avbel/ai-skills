#!/bin/bash
#
# review.sh — delegate a code review of local git state to the Codex CLI (`codex`).
#
# Mirrors the Gemini/Claude review skills: detect the review scope from git,
# capture the exact diff into a self-contained review brief, then hand the brief
# to `codex exec` running non-interactively and stream its verdict to stdout.
#
# The review is FORCED to run at xhigh reasoning effort
# (-c model_reasoning_effort=xhigh) on a GPT (OpenAI) model via Codex,
# independent of the caller's own session model.
#
# Usage:
#   review.sh [--scope auto|working-tree|branch] [--base <ref>] [--adversarial]
#             [--model <id>] [--timeout <duration>] [focus text ...]
#
# Output (stdout): the review report from `codex`, verbatim.
...
# The forced review configuration. xhigh effort is the point of this skill: a
# maximal-effort second opinion. The model defaults to Codex's configured model
# (always a GPT/OpenAI model) so the skill never breaks on a stale model id;
# pin one with --model or the CODEX_REVIEW_MODEL env var when you need a specific
# GPT-5.x-codex build (xhigh is model-dependent — a model that predates it errors).
REVIEW_EFFORT="xhigh"
REVIEW_MODEL="${CODEX_REVIEW_MODEL:-}"

# Max bytes of diff to embed in the prompt. The brief is piped to `codex exec`
# on stdin, but cap it anyway to protect the model's context window.
...
if ! command -v codex >/dev/null 2>&1; then
  echo "Error: 'codex' (Codex CLI) is not installed or not in PATH." >&2
  echo "Install it (npm i -g @openai/codex, or see the Codex docs) and sign in, then retry." >&2
  exit 127
fi
...
# `codex exec` has no built-in timeout, so wrap it with coreutils timeout when
...
if command -v timeout >/dev/null 2>&1; then
  ...
  echo "Warning: no 'timeout'/'gtimeout' found; running codex without a hard timeout." >&2
fi
...
MODEL_LABEL="${REVIEW_MODEL:-<codex default>}"
echo "Scope: $EFFECTIVE_SCOPE | Target: $TARGET_LABEL" >&2
echo "Building review brief and invoking codex (model=$MODEL_LABEL, effort=$REVIEW_EFFORT, timeout $TIMEOUT)..." >&2
...
# ---- build the brief (in-memory; piped to codex on stdin) -----------------
BRIEF="$(
  cat <<EOF
# Code Review Brief
...
# ---- run the review -------------------------------------------------------
# Security posture: the precise change is piped INLINE on stdin (`codex exec -`),
# and Codex runs in an OS-enforced READ-ONLY sandbox (--sandbox read-only): it can
# read the repository for context (callers, types, tests) but cannot write, create,
# delete, or reach the network. Non-interactive `codex exec` also forces
# approval_policy=never, so nothing escalates out of the sandbox. This is safe for
# reviewing YOUR OWN changes; for UNTRUSTED third-party code, run inside a
# container/VM with only the repo mounted (see SKILL.md "Security"). The prompt
# additionally fences the diff as untrusted data against injection.
CODEX_CMD=(codex exec --cd "$GIT_ROOT" --sandbox read-only --color never)
if [ -n "$REVIEW_MODEL" ]; then
  CODEX_CMD+=(--model "$REVIEW_MODEL")
fi
CODEX_CMD+=(-c "model_reasoning_effort=$REVIEW_EFFORT" -)

if [ ${#TIMEOUT_CMD[@]} -gt 0 ]; then
  printf '%s' "$BRIEF" | "${TIMEOUT_CMD[@]}" "${CODEX_CMD[@]}"
else
  printf '%s' "$BRIEF" | "${CODEX_CMD[@]}"
fi
