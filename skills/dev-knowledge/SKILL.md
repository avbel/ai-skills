...
- Keep investigation history and extended trade-offs in the note. Source code stays self-documenting per `dev-code-style`; retain a brief local explanation of any constraint an editor needs, with a link to the note when useful. Do not replace an essential local invariant with only a distant link or copy the note into comment blocks.
