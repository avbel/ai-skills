...
2. The OTel JS SDK 2.x line (current stable `2.10.0`; `@opentelemetry/sdk-node` ≥ 0.200, currently `0.221.0`; `@opentelemetry/auto-instrumentations-node` currently `0.79.0`) requires Node `^18.19.0 || >=20.6.0`. Older Node 14/16/18.0–18.18 are unsupported — verify the runtime before debugging missing spans. CI now also builds on Node 26, but Node 26 is not yet an officially supported runtime.
...
The HTTP, Fetch, XMLHttpRequest, and gRPC instrumentations now emit only stable HTTP / network semantic conventions (this is final since `auto-instrumentations-node` 0.79 / `sdk-node` 0.221). `OTEL_SEMCONV_STABILITY_OPT_IN` no longer changes HTTP attribute or metric emission, and the per-instrumentation `semconvStabilityOptIn` option has been removed — the old (`v1.7.0`) and duplicate (`http`, `http/dup`) semconv outputs are gone entirely. If dashboards or alerts depended on the old attribute names (e.g. `http.method` instead of `http.request.method`), update them before upgrading to `auto-instrumentations-node` 0.79.
...
Prefer W3C `tracecontext` (the default). The Jaeger propagator (`@opentelemetry/propagator-jaeger`) is formally deprecated as of stable `2.10.0` / experimental `0.221.0` — selecting it via `OTEL_PROPAGATORS` or declarative config now emits a deprecation warning, and the package will be removed in JS SDK 3.x (planned for ~September 2026).
